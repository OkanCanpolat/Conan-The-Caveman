...
TileLayer::TileLayer(int tileSize, int mapWidth, int mapHeight, const
...
void TileLayer::render(Player* player)
{
	int x, y, x2, y2 = 0;
	...
	for (int i = 0; i < m_numRows; i++)
	{
		for (int j = 0; j < m_numColumns; j++)
		{
			...
			if (((j * m_tileSize) - x2) - TheCamera::Instance()->
				getPosition().m_x < -m_tileSize || 
				((j * m_tileSize) - x2) - TheCamera::Instance()->
				getPosition().m_x > TheGame::Instance()->getGameWidth())
			{
				continue;
			}
			...
			TheTextureManager::Instance()->drawTile(tileset.name, 
				...
				((j * m_tileSize) - x2) - TheCamera::Instance()->
				getPosition().m_x, ((i * m_tileSize) - y2) - TheCamera::Instance()->
				getPosition().m_y, m_tileSize,
